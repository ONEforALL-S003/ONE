...
def check_operator(test: dict, compare: dict, test_path: str, compare_path: str):
    ...
    for key in test_keys:
        if key == 'weight':
            check_np_shape(
                os.path.join(test_path, test[key]),
                os.path.join(compare_path, compare[key]))
        elif key == 'zerop':
            check_np_shape(
                os.path.join(test_path, test[key]),
                os.path.join(compare_path, compare[key]))
...
if __name__ == "__main__":
    ...
    for testcase in args.testcases:
        ...
        test_mapped_list = [mapper.get_mapped_dict(), [None, None]]
        ...
        for i in range(2):
            ...
            json_paths = []
            for j in range(2):
                json_paths.append(current_test_path + os.sep + json_prefix[j] +
                                  'qparm.json')
                json_paths.append(current_compare_path + os.sep + json_prefix[j] +
                                  'qparm.json')
            ...
            for j in range(2):
                try:
                    # if only 1 file exists → test fails
                    if os.path.exists(json_paths[2 * j]) ^ os.path.exists(
                            json_paths[2 * j + 1]):
                        raise Exception
                    # if both files not exist → ok
                    ...
                    # if test and compare file's key different → test fail
                    if (test_keys & compare_keys) != test_keys:
                        raise Exception
                    ...
                    if j == 0:
                        flag |= 0b01
                    # without mapping fails
                    elif j == 1:
                        flag |= 0b10
            # testcase ok
            if flag == 0b00:
                continue
            # with mapping fails → extracting quantization ok, but fail to mapping it correctly
            # It could happen with different library version. (torch, onnx_tf, tf)
            elif flag == 0b01:
                raise Exception(
                    'Test fails due to different mapping. (Maybe different onnx_tf version)'
                )
            else:
